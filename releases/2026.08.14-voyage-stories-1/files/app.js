...
function beginMapPointEditor(latlng) {
  mapPointPickActive = false;
...
function initializeMapPointTool() {
  ...
  document.getElementById("map-point-add-btn")?.addEventListener("click", () => {
    mapPointPickActive = true; pendingMapPointPosition = null;
    document.getElementById("map").style.cursor = "crosshair";
    document.getElementById("map-point-instruction").textContent = "Click the map where the new point belongs.";
  });
  ...
  document.getElementById("map-point-save-btn")?.addEventListener("click", () => {
    ...
    savedMapPoints.push({ id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, ...pendingMapPointPosition, symbol: symbol.value, customUrl, size: Number(size.value), name: document.getElementById("map-point-name").value.trim(), description: document.getElementById("map-point-description").value.trim() });
...
async function createSailvuFeedback() {
  ...
  try {
    const payload = {
      ...
      app: { schema: "sailvu.feedback.v1", createdAt: new Date().toISOString() },
